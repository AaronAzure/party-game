...
{
    [CustomActionEditor(typeof(BT_Rotate))]
    public class BT_RotateEditor : PlayMakerTransitionGameObjectBaseEditor
    {
        /// <summary>
        ...
        protected override void ShowFooterGUI() {
            ...
            var playMakerRotate = target as BT_Rotate;
            EditorGUILayout.LabelField("Rotate Specific", EditorStyles.boldLabel);
            ShowSpecificSharedGUI();
            EditField("RotationMode");
            EditField("TransitionMode");
            if (playMakerRotate.TransitionMode != PlayMakerTransitionBase.TransitionModeType.FromCurrent)
            {
                EditField("StartRotation");
            }
            if (playMakerRotate.TransitionMode != PlayMakerTransitionBase.TransitionModeType.ToCurrent)
            {
                EditField("EndRotation");
            }
        }
