...
{
    [CustomActionEditor(typeof(BT_Fade))]
    public class BT_FadeEditor : PlayMakerTransitionGameObjectBaseEditor
    {
        /// <summary>
        ...
        protected override void ShowFooterGUI() {
            ...
            var playMakerFade = target as BT_Fade;
            EditorGUILayout.LabelField("Fade Specific", EditorStyles.boldLabel);
            ShowSpecificSharedGUI();
            EditField("TransitionMode");
            if (playMakerFade.TransitionMode != PlayMakerTransitionBase.TransitionModeType.FromCurrent)
            {
                EditField("StartTransparency");
            }
            if (playMakerFade.TransitionMode != PlayMakerTransitionBase.TransitionModeType.ToCurrent)
            {
                EditField("EndTransparency");
            }
        }
