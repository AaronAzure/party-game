...
{
    [CustomEditor(typeof(TransitionScale))]
    public class TTransitionScaleEditor : TransitionGameObjectBaseEditor
    {
        SerializedProperty _inStartScaleProperty;
        SerializedProperty _outEndScaleProperty;

        protected override void OnEnable()
        {
            var inConfigProperty = serializedObject.FindProperty("InConfig");
            _inStartScaleProperty = inConfigProperty.FindPropertyRelative("StartScale");
            var outConfigProperty = serializedObject.FindProperty("OutConfig");
            _outEndScaleProperty = outConfigProperty.FindPropertyRelative("EndScale");
            base.OnEnable();
        }
        ...
        protected override void ShowTransitionInGUI() {
            ...
            EditorGUILayout.PropertyField(_inStartScaleProperty);
        }
        ...
        protected override void ShowTransitionOutGUI() {
            GUILayout.Space(3f);
            EditorGUILayout.LabelField("Scale Specific", EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(_outEndScaleProperty);
        }
    }
}
