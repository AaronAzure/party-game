...
{
    [CustomActionEditor(typeof(BT_Volume))]
    public class BT_VolumeEditor : PlayMakerTransitionGameObjectBaseEditor
    {
        /// <summary>
        ...
        protected override void ShowFooterGUI() {
            ...
            var playMakerVolume = target as BT_Volume;
            EditorGUILayout.LabelField("Volume Specific", EditorStyles.boldLabel);
            ShowSpecificSharedGUI();
            EditField("TransitionMode");
            if (playMakerVolume.TransitionMode != PlayMakerTransitionBase.TransitionModeType.FromCurrent)
            {
                EditField("StartVolume");
            }
            if (playMakerVolume.TransitionMode != PlayMakerTransitionBase.TransitionModeType.ToCurrent)
            {
                EditField("EndVolume");
            }
        }
