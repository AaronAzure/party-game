...
{
    [CustomEditor(typeof(TransitionVolume))]
    public class TransitionVolumeEditor : TransitionGameObjectBaseEditor
    {
        SerializedProperty _inStartVolumeProperty;
        SerializedProperty _outEndVolumeProperty;

        protected override void OnEnable()
        {
            var inConfigProperty = serializedObject.FindProperty("InConfig");
            _inStartVolumeProperty = inConfigProperty.FindPropertyRelative("StartVolume");
            var outConfigProperty = serializedObject.FindProperty("OutConfig");
            _outEndVolumeProperty = outConfigProperty.FindPropertyRelative("EndVolume");
            base.OnEnable();
        }
        ...
        protected override void ShowTransitionInGUI() {
            GUILayout.Space(3f);
            EditorGUILayout.LabelField("Volume Specific", EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(_inStartVolumeProperty);
        }
        ...
        protected override void ShowTransitionOutGUI() {
            GUILayout.Space(3f);
            EditorGUILayout.LabelField("Volume Specific", EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(_outEndVolumeProperty);
        }
    }
}
